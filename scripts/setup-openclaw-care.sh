...
setup_docker_mode() {
  ...
  upsert_env "OPENCLAW_AGENT_ID" "care"
...
setup_standalone_mode() {
  ...
  mkdir -p \
    "$OPENCLAW_HOME/agents/care/agent" \
    "$OPENCLAW_HOME/agents/care/sessions" \
    "$OPENCLAW_HOME/workspace-care"
  ...
  cp -r "$ROOT/openclaw/workspace-care/." "$OPENCLAW_HOME/workspace-care/"
  sed "s/__OPENCLAW_GATEWAY_TOKEN__/$token/" "$ROOT/openclaw/openclaw.care.json5" > "$config_file"

  upsert_env "OPENCLAW_GATEWAY_URL" "http://127.0.0.1:18789"
  upsert_env "OPENCLAW_GATEWAY_TOKEN" "$token"
  upsert_env "OPENCLAW_AGENT_ID" "care"

  echo "OpenClaw care configurado en $OPENCLAW_HOME"
...
echo "    -d '{\"model\":\"openclaw/care\",\"messages\":[{\"role\":\"user\",\"content\":\"Hola\"}]}'"
